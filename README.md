...
- **다크모드** — 헤더 토글(해/달)로 라이트/다크 전환. 미선택 시 OS 설정을 따르고, 선택은 `localStorage` 에 기억합니다. `darkMode: "class"` + CSS 변수 팔레트(`:root`/`.dark`) 기반이라 컴포넌트마다 `dark:` 를 붙이지 않아도 됩니다. 로고도 라이트=검정/다크=흰색 SVG 로 자동 전환되고, giscus 댓글 테마도 연동됩니다.
